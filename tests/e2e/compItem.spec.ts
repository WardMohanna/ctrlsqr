...
test("add item to half completed category, check if its there, delete it.", async ({ page }) => {
    //for now this test is filling the weight by 120grams 
    //recommended for the future to check the weight from the tree 
    //then update the weight as it should be automatically



    const itemCode = 'comp item';
    const itemBarcode = 'comp barcode';
    ...
    await page.locator('div[role="option"]', { hasText: 'מוצר גמור'}).click();
    ...
    //price before tax
    await input.nth(7).fill('16');

    //recommended price to sell
    await input.nth(8).fill('19');

    //item weight
    await input.nth(9).fill('120');

    //choose and filling the tree 
    await input.nth(10).click();
    ...
    await input.nth(11).fill('60');
    await input.nth(12).fill('60');
