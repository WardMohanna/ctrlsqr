    ...
    const handleMouseMove = (e) => {
      ...
      if (enableMagnetism) {
        ...
        magnetismAnimationRef.current = gsap.to(element, {
          x: magnetX,
          y: magnetY,
          duration: MAGNETISM_FOLLOW_DURATION,
          ease: "none",
        });
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      gsap.to(spotlightRef.current, {
        ...
        duration: targetOpacity > 0 ? 1.1 : 0.5,
        ease: "power2.out",
      });
    };
...
const BentoCardGrid = ({ children, gridRef, gridClassName = "" }) => (
  <div
    className={`card-grid bento-section ${gridClassName}`.trim()}
    ref={gridRef}
  >
    {children}
  </div>
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef} gridClassName={gridClassName}>
        {items.map((card, index) => {
          const baseClassName =
            `magic-bento-card ${textAutoHide ? "magic-bento-card--text-autohide" : ""} ${enableBorderGlow ? "magic-bento-card--border-glow" : ""} ${cardClassName}`.trim();
          const resolvedStyle =
            typeof getCardStyle === "function"
